...
enum RadarService {
    ...
    static func interactiveHTML(lat: Double, lon: Double, zoom: Int = 8, isDark: Bool = true) -> String {
        let basemap = isDark
            ? "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png"
            : "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png"
        let mapBg = isDark ? "#0b0f14" : "#f0f0f0"
        let ctrlBg = isDark ? "rgba(15,20,28,.92)" : "rgba(255,255,255,.92)"
        let ctrlText = isDark ? "#cfe3ff" : "#333333"
        let btnBg = isDark ? "#1d6fe0" : "#1d6fe0"
        let btnOffBg = isDark ? "#333a44" : "#e0e0e0"
        let btnOffText = isDark ? "#8aa0b8" : "#666666"
        let modeBg = isDark ? "rgba(15,20,28,.92)" : "rgba(255,255,255,.92)"
        let modeBorder = isDark ? "#2a3545" : "#cccccc"
        let modeActiveBg = isDark ? "rgba(29,111,224,.3)" : "rgba(29,111,224,.15)"
        let labelBg = isDark ? "rgba(15,20,28,.8)" : "rgba(255,255,255,.85)"
        let labelColor = isDark ? "#e0a21d" : "#b8860b"
        let labelBorder = isDark ? "rgba(224,162,29,.4)" : "rgba(184,134,11,.4)"
        let markerColor = "#1d6fe0"
        ...
        <style>
          html,body,#map{height:100%;margin:0;background:\(mapBg);}
          .ctrl{position:absolute;z-index:1000;bottom:12px;left:12px;background:\(ctrlBg);
                color:\(ctrlText);padding:10px 14px;border-radius:10px;font:13px -apple-system,system-ui;
                display:flex;gap:10px;align-items:center;}
          .ctrl button{background:\(btnBg);color:#fff;border:0;border-radius:6px;padding:6px 12px;
          ...
          .mode-sw button{background:\(modeBg);color:\(btnOffText);border:1px solid \(modeBorder);
                          border-radius:8px;padding:6px 10px;font-size:11px;cursor:pointer;}
          .mode-sw button.active{color:#fff;border-color:#1d6fe0;background:\(modeActiveBg);}
          .wind-label{background:\(labelBg);color:\(labelColor);padding:1px 4px;border-radius:3px;
                      font:bold 10px -apple-system;white-space:nowrap;border:1px solid \(labelBorder);}
        </style>
        </head>
        <body>
        <div id="map"></div>
        ...
        <script>
          ...
          L.tileLayer('\(basemap)',{maxZoom:19}).addTo(map);
          L.circleMarker([LAT,LON],{radius:6,color:'\(markerColor)',fillOpacity:0.9,weight:2}).addTo(map);
                ...
                .then(obs=>{
                  ...
                  const arrowSvg=`<svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
                    <g transform="rotate(${wd+180},20,20)">
                      <line x1="20" y1="6" x2="20" y2="34" stroke="\(labelColor)" stroke-width="2"/>
                      <polygon points="20,4 15,14 25,14" fill="\(labelColor)"/>
                    </g>
                  </svg>`;
                  const icon=L.divIcon({
                    html:arrowSvg+`<div class="wind-label">${Math.round(mph)}</div>`,
                    className:'',iconSize:[40,55],iconAnchor:[20,20]
                  });
